...
{if $node.data_map.jumbo.data_int}

{else}
{def $nodes=fetch( 'content', 'list',
        hash( 'parent_node_id', 120,
              'depth', 1 ) )} 
{foreach $nodes as $node1}
    <input type="checkbox" name="{$node1.name|wash}" value="Bike">{$node1.name|wash} {attribute_view_gui attribute=$node1.data_map.price}<br>
{/foreach}  
{/if}
